...
struct MyProfileView: View {
    ...
    @State private var showChangePlayer: Bool = false
    ...
    var body: some View {
        NavigationStack {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Menu {
                        Button {
                            ...
                            Label("Change Player", systemImage: "arrow.triangle.2.circlepath")
                        }
            ...
            .onChange(of: authService.playerAccountId) { _, newValue in
                if newValue != nil {
    ...
    private func loadMyProfile() async {
        guard let accountId = authService.playerAccountId else { return }
    ...
    private var scavStatsSection: some View {
        let scav = viewModel.scavStats ?? PlayerStats(sessions: 0, survived: 0, kills: 0, deaths: 0, totalInGameTime: 0)
                ...
                VStack(alignment: .leading, spacing: 14) {
                    HStack {
                        Image(systemName: "figure.walk")
                            .foregroundStyle(tarkovGold)
                        Text("Scav Statistics")
                            .font(.headline)
                    }

                    VStack(spacing: 0) {
                        statsRow(label: "Total Raids", value: "\(scav.sessions)")
                        Divider().padding(.leading, 16)
                        statsRow(label: "Survived", value: "\(scav.survived)")
                        Divider().padding(.leading, 16)
                        statsRow(label: "Kills", value: "\(scav.kills)")
                        Divider().padding(.leading, 16)
                        statsRow(label: "K/D Ratio", value: String(format: "%.2f", scav.kd))
                        Divider().padding(.leading, 16)
                        statsRow(label: "Survival Rate", value: String(format: "%.1f%%", scav.survivalRate))
                    }
                    .adaptiveGlass(in: .rect(cornerRadius: 14))
                }
            }
